...
const allowedOrigins = [
  'http://127.0.0.1:5500',
  'http://localhost:5500',
  'https://fundilink-frontend.onrender.com' // your production frontend URL
];
...
// ✅ Universal Login (Fundi or Client)
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.password) return res.status(500).json({ error: 'Password missing' });
...
app.delete('/api/fundis/:id', async (req, res) => {
  try {
    const fundi = await Fundi.findByIdAndDelete(req.params.id);
